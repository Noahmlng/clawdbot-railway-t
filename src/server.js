  ...
  <div class="card">
    ...
    <div style="display:flex; gap:0.5rem; align-items:center">
      <select id="consoleCmd" style="flex: 1">
        ...
        <option value="openclaw.plugins.install">openclaw plugins install &lt;packageOrName&gt;</option>
      </select>
...
app.post("/setup/api/console/run", requireSetupAuth, async (req, res) => {
  ...
  try {
    ...
    if (cmd === "openclaw.plugins.install") {
      const plugin = String(arg || "").trim();
      if (!plugin) return res.status(400).json({ ok: false, error: "Missing plugin package/name" });
      if (plugin.length > 128) return res.status(400).json({ ok: false, error: "Plugin package/name too long" });
      if (!/^[A-Za-z0-9_@./-]+$/.test(plugin)) return res.status(400).json({ ok: false, error: "Invalid plugin package/name" });
      const r = await runCmd(OPENCLAW_NODE, clawArgs(["plugins", "install", plugin]));
      return res.status(r.code === 0 ? 200 : 500).json({ ok: r.code === 0, output: redactSecrets(r.output) });
    }
